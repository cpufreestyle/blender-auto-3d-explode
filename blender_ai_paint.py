...
"""
    ...
    try:
        if provider == 'openai':
            ...
            result = call_nvidia_api(config['nvidia'], system_prompt, user_prompt)
...
def call_stepfun_api(config, system_prompt, user_prompt):
    ...
    # 兼容旧模型名称，映射到可用的模型
    old_models = ['step-1-8k', 'step-1-32k', 'step-1-128k', 'step-1-flash', 
                  'step-1o-vision-32k', 'step-1o-turbo-vision', 'step-2x-large']
    if model in old_models:
        log(f"  ⚠️ 模型 {model} 不可用，切换到 step-3.5-flash")
...
def call_nvidia_api(config, system_prompt, user_prompt):
    """调用 NVIDIA API"""
    import urllib.request
    import json
    ...
    if not api_key:
        raise Exception('NVIDIA API Key 未配置')
...
def create_lego_brick(brick_type, position, rotation, color_name, name, parts):
    ...
    # 砖块尺寸定义 (单位: cm)
    brick_sizes = {
        '2x4': (3.2 * SCALE, 1.6 * SCALE, 0.96 * SCALE),  # 长, 宽, 高(不含凸点)
        '2x2': (1.6 * SCALE, 1.6 * SCALE, 0.96 * SCALE),
        '1x2': (1.6 * SCALE, 0.8 * SCALE, 0.96 * SCALE),
        '1x4': (3.2 * SCALE, 0.8 * SCALE, 0.96 * SCALE),
        '2x3': (2.4 * SCALE, 1.6 * SCALE, 0.96 * SCALE),
        '1x1': (0.8 * SCALE, 0.8 * SCALE, 0.96 * SCALE),
    }
    ...
    # 添加倒角模拟乐高圆角
    apply_bevel_mod(brick, width=0.015 * SCALE, segments=2)
    
    parts.append(add_obj(brick, name, mat))
    
    # 添加凸点 (studs)
    stud_radius = 0.24 * SCALE  # 凸点半径
    stud_height = 0.18 * SCALE  # 凸点高度
    stud_spacing = 0.8 * SCALE  # 凸点间距
    ...
    for i in range(studs_x):
        for j in range(studs_y):
            ...
            apply_bevel_mod(stud, width=0.005 * SCALE, segments=1)
...
def match_prompt(prompt):
    ...
    if best_match:
        ...
        # 乐高风格为特殊标记，走 create_lego_style（避免返回字符串导致调用方 TypeError）
