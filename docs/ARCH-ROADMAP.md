...
- `main.js` 当前实测 909 行（2026-09-25）：L2 前端模块化已完成（见下方 L2 行），3D 拆解控制、几何拆分、相机适配、样式切换、自定义模型加载/面板/收尾、模型归一化、拆卸释放、装配顺序对接、Quest 3 默认模型构建、GLTF 惰性加载均已迁至 `src/` 下 ES module；剩余为场景初始化、渲染循环、DOM 状态条与事件监听、各实例的 getState/setState 桥接等应用外壳代码。
