...
| L2 | `main.js` 前端模块化（按 3D 视图/上传/配置面板/VLM 流程拆 ES module） | Could | 🟡 试点完成：几何体拆分纯函数（extractFacesToGeometry / splitBy* / generatePartName）迁入 `src/geometry-split.js`，`main.js` 减 ~150 行、行为不变；剩余（视图/上传/配置/VLM 面板）因共享状态耦合 + 无法在此环境跑 Three.js 预览，放最后单独做 |
