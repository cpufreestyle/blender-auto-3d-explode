...
| 清理 VLM 生成产物 | `scripts/_vlm_generated_blender.py` 移出版本控制（`git rm` + `.gitignore`）；残留引用只有两处解释历史的注释，无功能依赖 | 本轮 / #19 |
| CI Blender 冒烟门禁 | 新增第三个 CI job `blender-smoke`：装与开发机同版本的官方 Blender 5.1.2（tarball 缓存），跑 `blender_split_glb.py` 拆分（断言 15 部件 / GLB 体积）与 `quest3_exploded_blender.py` 程序化建模（断言 15 个模型对象 / 材质节点树完好，即 use_nodes 迁移的运行时守卫）。断言逻辑收在 `scripts/ci_blender_smoke.py`，本机可复现 | 本轮 / #21 |
...
| ~~Later~~ | ~~清理 `scripts/_vlm_generated_blender.py`~~ | ✅ 已完成（见 §1）：生成产物已移出版本控制并加 `.gitignore`；`vlm_img_to_blender.py` 的 `--code-out` 每请求唯一、结束即清理 |
...
# 5) Blender 冒烟（CI blender-smoke job 的本机复现；期望「冒烟结果: PASS」）
